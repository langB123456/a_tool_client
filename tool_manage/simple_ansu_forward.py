...
import pytesseract, os, pyautogui, cv2, random, string
import numpy as np
import subprocess
...
# 登出安速
def logout(base_url, authorization):
    url = base_url + "/api/base/biz/user/logout"
...
def system_download(base_url, user, pwd):
    ...
    # authorization = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..HetGEgtpfaq4y5W3esnpqpoj-j-Qy-jW3GEZLlMwbbA'
    time.sleep(0.5)
...
def sales_match_group():
    ...
    for i in range(len(select_data)):
        for j in range(2, deal_row_count + 1):
            if select_data[i] == deal_ws.cell(row=j, column=4).value and deal_ws.cell(row=j,
                                                                                      column=4).value is not None:
                k += 1
...
def send_result_for_me(user_name, forward_count, success_count, start_time, end_time):
    ...
    get_picture('D:\\group_send\\picture_manage\\search_box_photo.png', 200, 20, 200, 50)
    # 搜索输入框查询
...
def new_main_use_flow(result_send):
    try:
        ...
        for i in range(0, len(user_names)):
            get_picture('D:\\group_send\\picture_manage\\search_box_photo.png', 200, 15, 200, 50)
        ...
        wb.save(deal_excel)
        send_result_for_me(result_send, forward_count - 1, success_count, start_time, end_time)
    except Exception as e:
        print(e)
    # finally:
    #     wake_up_my_client()
...
def cs_forward_main(base_url, result_send, user, pwd):
...
def tool_ansu_simple_cs_window(tool):
    ...
    # 右边功能窗口
    r1 = tk.Label(tool_use_page, text='安速客服 根据运单号 逐个发送(个例)', background='white', justify='left')
    r1.place(x=0, y=0)
    ...
    e1.insert(0, 'http://fba.ansuex.com/')
    e1.place(x=100, y=28)
