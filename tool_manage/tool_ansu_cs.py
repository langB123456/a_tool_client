...
def system_download(base_url, user, pwd):
    ...
    time.sleep(0.5)
    shipment_nums = get_shipment_num_list('D:\\group_send\\运单号.xlsx', 'Sheet1')
    ...
    if export_file(base_url, authorization, shipment_nums, biz_export_template_id):
        time.sleep(0.5)
        download_url = get_news_list(base_url, authorization, '测试aaa')
        time.sleep(0.5)
        download_file(download_url, 'D:\\group_send\\', '安速-客服-数据发送.xlsx')
...
def sales_match_group():
    deal_path = 'D:\\group_send\\安速-客服-数据发送.xlsx'
    read_wb = openpyxl.load_workbook('D:\\group_send\\安速客服群发-基础信息维护.xlsx')
    read_ws = read_wb['群名称']
...
def main_use_flow():
    try:
        ...
        # 输入转发群名称
        deal_excel = 'D:\\group_send\\安速-客服-数据发送.xlsx'
...
def cs_forward_main(base_url, user, pwd):
    ...
    time.sleep(1)
    if os.path.exists(r'D:\\group_send\\安速-客服-数据发送.xlsx'):
        os.remove(r'D:\\group_send\\安速-客服-数据发送.xlsx')
