    ...
    try:
        ...
        for i in range(len(excel_vat)):
            if str(pdf_vat) == str(excel_vat[i]):
                row_num = i + 1
                is_deal_data = ws.cell(row_num, 17).value
                if is_deal_data == "已处理":
                    continue
                row = []
...
# 进入sheet
def to_sheet(workbook, worksheet):
    now_worksheet = workbook.get_sheet_by_name(worksheet)
    return now_worksheet
...
# 创建单号excel的sheet及表头
def create_excel_data(excel_path):
    try:
        wb = load_workbook(excel_path)
        # 包税sheet
        ...
        for i in range(0, len(header_row)):
            ws.cell(row=1, column=i + 1, value=header_row[i])
        # 问题件sheet
        wb.create_sheet("问题件", len(wb.sheetnames) + 1)
        ws = wb['问题件']
        header_row = ["提单号", "VAT号", "件数", "税金", "分单号"]
        for i in range(0, len(header_row)):
            ws.cell(row=1, column=i + 1, value=header_row[i])
...
# 判断是否存在vat
def is_vat_exict(pdf_vat, pdf_nums, pdf_declarat_values, pdf_Invoice_total, folder_path, remote_file):
    try:
        # excel文件
        ...
        if str(pdf_vat) in excel_vat:
            ...
            if excel_data == None:
                problem_deal(pdf_vat, pdf_nums, pdf_declarat_values, pdf_Invoice_total, folder_path, remote_file,
                             excel_path)
                return
                # 不包税 判断vat、件数、申报价值 所有条件相等
            if str(pdf_vat) == str(pdf_vat) and int(pdf_nums) == int(excel_data[4]) and float(pdf_declarat_values) == float(
                    excel_data[5]):
                is_vat_one_all_equal(pdf_vat, pdf_vat, pdf_nums, excel_data[4], pdf_declarat_values, excel_data[5],
                                     remote_file, folder_path, excel_data[6], pdf_Invoice_total)
                print("不包税处理1")
            else:
                # 不包税 vat相等（多条）
                excel_data_list = get_vat_excel_data_list(excel_path, "Sheet1", pdf_vat,
...
def is_vat_list_equal(pdf_vat, pdf_nums, excel_nums, pdf_declarat_values, excel_declarat_values, remote_file,
    ...
    if int(pdf_nums) == int(excel_nums) and round(float(pdf_declarat_values), 2) == round(float(excel_declarat_values),
        ...
        # 问题件记录
        problem_deal(pdf_vat, pdf_nums, pdf_declarat_values, pdf_Invoice_total, folder_path, remote_file, excel_path)
        print("问题件记录")
...
def tax_off(remote_file, folder_path, row_num, pdf_Invoice_total):
    try:
        ...
        ws.cell(last_row, 16, vat_num)
        ws.cell(last_row, 17, sale_name)
        ws.cell(last_row, 18, finance_name)
        wb.save(excel_path)
        # 重命名
...
# 问题件处理
def problem_deal(pdf_vat, pdf_nums, pdf_declarat_values, pdf_Invoice_total, folder_path, remote_file, excel_path):
    try:
        wb = load_workbook(excel_path)
        ws = wb['Sheet1']
        # 获取提单号
        ...
        ws.cell(last_row, 4, pdf_Invoice_total)
        wb.save(excel_path)
        new_name = "问题件 " + pdf_vat + " " + pdf_nums + " " + pdf_Invoice_total + ".pdf"
        new_name = (folder_path + "\\" + new_name).replace("\\", "\\\\")
        if os.path.exists(new_name):
            new_name = new_name.replace(".pdf", " 1.pdf")
        os.rename(remote_file, new_name)
...
def main_deal_excel():
    try:
        ...
        for i in range(len(folder_paths)):
            ...
            # 给单号.xlsx添加sheet
            create_excel_data((folder_path + '\\单号.xlsx').replace("\\", "\\\\"))
            ...
            for i in range(len(file_paths)):
                ...
                is_vat_exict(pdf_vat, pdf_nums, pdf_declarat_values, pdf_Invoice_total, folder_path, pdf_path)
                print(pdf_vat, pdf_nums, pdf_declarat_values, pdf_Invoice_total, folder_path, pdf_path)
    except Exception as e:
        print(e)
        kill_processes_by_names(['wps.exe'])
    finally:
...
if __name__ == '__main__':
    data = get_pdf_data("D:\\yingdaoFile\\demand3\\testFile\\232-12437585\\GB284395956 1 0.pdf")
    tax_id = get_Tax_ID_old(data)
    print(tax_id)